...
## @brief Weight conversion script for ScreenAI caption model
##        (SigLIP2 encoder + BERT cross-attention decoder).
...
# Order MUST match CaptionBertDecoder layer creation order (Tasks 4–6).
#
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert caption_s02 safetensors checkpoint to nntrainer format."
    )
    parser.add_argument(
        "--model_path",
        type=str,
        default="model.safetensors",
        help="Path to caption_s02/model.safetensors",
    )
    parser.add_argument(
        "--encoder_output",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Which weight file(s) to produce",
    )
    parser.add_argument(
        "--safetensors",
        action="store_true",
...
def main():
    """Convert caption_s02 checkpoint to nntrainer weight files."""
    args = parse_args()
