...
void Lfm2CausalLM::run_with_embeddings(const void *inputs_embeds,
  ...
  // input_sample is padded to INIT_SEQ_LEN above whenever needs_padding is
  // true; otherwise input_len == INIT_SEQ_LEN (enforced above), so the
  // buffer is always exactly this size either way.
  const size_t input_sample_bytes =
    sizeof(float) * static_cast<size_t>(BATCH_SIZE) *
    static_cast<size_t>(INIT_SEQ_LEN) * static_cast<size_t>(DIM);
  auto input = buildInferenceInputs(input_sample, input_sample_bytes);
