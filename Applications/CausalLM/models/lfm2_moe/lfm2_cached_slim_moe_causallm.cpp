 ...
 *
 * @file   lfm2_cached_slim_moe_causallm.cpp
 * @date   06 July 2026
...
Tensor Lfm2CachedSlimMoeCausalLM::createMoeLayer(const int layer_id,
                                                 Tensor input) {
  LayerHandle moe(createLayer(
    "lfm2_moe_cached_slim",
...
void Lfm2CachedSlimMoeCausalLM::registerCustomLayers() {
  ...
  try {
    app_context->registerFactory(
      nntrainer::createLayer<causallm::Lfm2CachedSlimMoELayer>);
  } catch (std::invalid_argument &e) {
    std::cerr << "failed to register Lfm2CachedSlimMoELayer factory, reason: "
              << e.what() << std::endl;
  }
}
