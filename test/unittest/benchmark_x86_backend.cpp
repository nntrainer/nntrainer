 ...
 */
class Bench_EleOps : public ::testing::TestWithParam<unsigned int> {};

TEST_P(Bench_EleOps, ele_sub) {
...
GTEST_PARAMETER_TEST(Sizes, Bench_EleOps,
                     ::testing::ValuesIn(g_bench_sizes));
 ...
 */
class Bench_Activations : public ::testing::TestWithParam<unsigned int> {};

TEST_P(Bench_Activations, softmax) {
  const unsigned int N = GetParam();
...
TEST_P(Bench_Activations, tanh_gelu) {
  const unsigned int N = GetParam();
  auto X = generate_random_vector<float>(N);
  std::vector<float> Y(N);
...
TEST_P(Bench_Activations, tanh_gelu_mul) {
  const unsigned int N = GetParam();
...
TEST_P(Bench_Activations, inv_sqrt_inplace) {
  const unsigned int N = GetParam();

  {
    ...
    auto stats = bench::measure(
      [&]() {
        ...
        nntrainer::inv_sqrt_inplace(N, X_tmp.data());
      },
      g_bench_warmup, g_bench_iters);
...
TEST_P(Bench_Activations, max_val) {
  const unsigned int N = GetParam();
...
#ifdef ENABLE_FP16
TEST_P(Bench_Activations, swiglu) {
  const unsigned int N = GetParam();
  auto Y_u16 = convert_f32_to_f16_u16(generate_random_vector<float>(N));
...
GTEST_PARAMETER_TEST(Sizes, Bench_Activations,
                     ::testing::ValuesIn(g_bench_sizes));
...
TEST_P(Bench_SoftmaxRow, softmax_row_inplace) {
  ...
  std::string sz = "rows=" + std::to_string(num_rows) +
 ...
 */
class Bench_FP16_BLAS : public ::testing::TestWithParam<unsigned int> {};

TEST_P(Bench_FP16_BLAS, sdot) {
  const unsigned int N = GetParam();
  auto X_u16 = convert_f32_to_f16_u16(generate_random_vector<float>(N));
  auto Y_u16 = convert_f32_to_f16_u16(generate_random_vector<float>(N));
...
GTEST_PARAMETER_TEST(Sizes, Bench_FP16_BLAS,
                     ::testing::ValuesIn(g_bench_sizes));
...
TEST_P(Bench_X86_FP16_HGEMM, via_sgemm_dispatch) {
  ...
  const std::size_t a_size =
    static_cast<std::size_t>(TransA ? K : M) * lda;
  const std::size_t b_size =
    static_cast<std::size_t>(TransB ? N : K) * ldb;
  ...
  auto fp16_stats = bench::measure(
    [&]() {
      ...
      nntrainer::sgemm(0, TransA, TransB, M, N, K, 1.0f,
...
GTEST_PARAMETER_TEST(HgemmDims, Bench_X86_FP16_HGEMM,
                     ::testing::Values(
                       HgemmParams{64, 64, 64, false, false},
...
GTEST_PARAMETER_TEST(GemvDims, Bench_FP16_GEMV,
                     ::testing::Values(GemvParams{1, 768},
                                       GemvParams{16, 1024},
                                       GemvParams{256, 1024},
                                       GemvParams{256, 3072},
...
static const auto kAttnConfigs = ::testing::Values(
  AttnParams{64, 8, 1, 128}, AttnParams{64, 8, 4, 128},
...
TEST_P(Bench_KCache, compute_kcaches) {
  ...
  std::vector<float> output(total_heads * seq_len, 0.0f);
...
TEST_P(Bench_Attention, compute_fp16vcache) {
  ...
  int row_num = (int)window_size;

  auto in_f32 = generate_random_vector<float>(total_heads * row_num);
  auto vcache_f32 =
    generate_random_vector<float>(num_cache_head * (int)window_size * head_dim);
  auto vcache_u16 = convert_f32_to_f16_u16(vcache_f32);
  std::vector<float> output(total_heads * head_dim, 0.0f);
...
TEST_P(Bench_KCache_FP16, compute_kcaches) {
  ...
  auto in_u16 =
    convert_f32_to_f16_u16(generate_random_vector<float>(total_heads * head_dim));
...
TEST_P(Bench_Attention_FP16, compute_fp16vcache_transposed) {
  ...
  auto in_u16 =
    convert_f32_to_f16_u16(generate_random_vector<float>(total_heads * row_num));
  auto vcache_u16 = convert_f32_to_f16_u16(
    generate_random_vector<float>(num_cache_head * (int)window_size * head_dim));
  ...
  auto stats = bench::measure(
    [&]() {
      nntrainer::compute_fp16vcache_transposed(
        row_num, (const _FP16 *)in_u16.data(),
        (const _FP16 *)vcache_u16.data(), (_FP16 *)output.data(),
        num_cache_head, gqa_size, head_dim, window_size);
    },
    g_bench_warmup, g_bench_iters);
  ...
  for (int i = 1; i < *argc; ++i) {
    if (strncmp(argv[i], "--bench_iters=", 14) == 0) {
      ...
      while (std::getline(ss, token, ',')) {
        ...
        if (v > 0)
          g_bench_sizes.push_back(static_cast<unsigned int>(v));
      }
    } else {
      argv[out++] = argv[i];
    }
