...
int nntr_hvx_mm_u8i4_from_f32(remote_handle64 handle, uint32 M, uint32 K,
  ...
  FARF(ALWAYS,
       "layout: w_size=%u act=[%u,%u) result_off=%u unshuf_off=%u "
       "config_off=%u",
       (unsigned)L.w_size, (unsigned)L.act_base,
       (unsigned)(L.act_base + L.act_size), (unsigned)L.result_off,
       (unsigned)L.unshuf_off, (unsigned)L.config_off);
...
int nntr_hvx_mm_u8i4_layer(remote_handle64 handle, uint32 M, uint32 K,
  ...
  return hexkl_mm_u8i4_layer_run(
    &s->weights, s->vtcm_base, s->vtcm_size, s->config_off, M, K, w_handles,
    (uint32_t)w_handlesLen, act_f32, out_cat, s->quant_pool);
}
