...
class CausalLMProfiler:
    ...
    def run_inference_benchmark(self) -> bool:
        ...
        try:
            # Set up environment with proper library paths for CausalLM and custom layers
            env = os.environ.copy()

            lib_dirs = []

            # Add CausalLM custom layer .so files (e.g., librms_norm_layer.so, libmha_core_layer.so)
            causallm_layers_dir = str(self.nntrainer_root / "build_docker" / "Applications" / "CausalLM" / "layers")
            if Path(causallm_layers_dir).is_dir():
                lib_dirs.append(causallm_layers_dir)

            # Add CausalLM main library directory
            causallm_dir = str(self.nntrainer_root / "build_docker" / "Applications" / "CausalLM")
            if Path(causallm_dir).is_dir():
                lib_dirs.append(causallm_dir)

            # Add nntrainer build and lib directories
            for lib_dir in [
                str(self.nntrainer_root / "build" / "nntrainer"),
                str(self.nntrainer_root / "lib"),
                str(self.nntrainer_root / "lib64"),
            ]:
