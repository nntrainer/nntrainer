...
Builds nntrainer from source on Ubuntu/Linux in two phases:
  python -m agents.build.nntrainer_builder <repo> <prefix> prep [--use-docker]
  python -m agents.build.nntrainer_builder <repo> <prefix> build [--use-docker]

"prep"  — validate repo, check tools, detect missing apt packages,
          sync git submodules. Returns apt commands only if needed.
          When using Docker, skips system package check.

"build" — detect build state, run meson + ninja with auto-retry
          (up to 2 retries before reporting failure).
          With --use-docker, runs the build inside a Docker container
...
# ------------------------------------------------------------------ docker support
def _log_docker_output(message: str, level: str = "info") -> None:
    """Log output from Docker builder with proper formatting."""
    if level == "error":
...
# ------------------------------------------------------------------ phases
def run_prep(repo: str, prefix: str, use_docker: bool = False) -> dict:
    ...
    if use_docker:
        bus.log("Using Docker build — skipping host system checks")
        if not docker_builder:
            msg = "Docker builder module not available"
            bus.log(msg, "error")
...
def run_build(repo: str, prefix: str, use_docker: bool = False) -> dict:
    ...
    if use_docker:
        ...
        bus.log("Building nntrainer in Docker container …")
        ok = docker_builder.build_nntrainer_in_docker(
            repo, prefix, log_fn=_log_docker_output
        )
        if not ok:
            msg = "Docker build failed — see log above"
            bus.log(msg, "error")
            bus.agent_status(AGENT_ID, "error", msg)
            return {"ok": False, "error": msg, "needs_user_help": True}
    else:
        ...
        meson_cmd = [
            "meson", "setup", "build", f"--prefix={prefix}",
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or sys.argv[3] not in ("prep", "build"):
        print("usage: python -m agents.nntrainer_builder <repo> <prefix> <prep|build> [--use-docker]")
        sys.exit(1)

    use_docker = "--use-docker" in sys.argv
    fn = run_prep if sys.argv[3] == "prep" else run_build
    print(json.dumps(fn(sys.argv[1], sys.argv[2], use_docker=use_docker)))
