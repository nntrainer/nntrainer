...
def run(state: dict) -> dict:
    ...
    if not state.get("weights_verified"):
        bus.log("Weights are not verified -- refusing raw weight conversion", "error")
        bus.agent_status("weight_converter", "error", "weights not verified")
        return state
    ...
    if cache.is_fresh(meta, "converted", WEIGHTS_CACHE_EXPIRY_DAYS) and os.path.exists(cached_path):
