...
def _kind(name: str) -> str:
    ...
    # Special handling for graph IR file
    if base == "nntrainer_graph_ir.json":
        return "Graph IR"
    
    # Special handling for CausalLM .bin files
    if ext == ".bin":
        if "nntr" in base or "causallm" in base or "model" in base:
            return "CausalLM Weights"
        return "Binary File"
