...
def run(state: dict) -> dict:
    ...
    # Publish graphs immediately so they appear in the webview without waiting for dual_graph agent
    model_view = state["model_graph_view"]
    nntrainer_view = state["nntrainer_graph_view"]
    ...
    # Save nntrainer_graph_ir as JSON file for inspection/debugging
    import json
    import os
    graph_ir_path = os.path.join(state["out_dir"], "nntrainer_graph_ir.json")
    with open(graph_ir_path, "w", encoding="utf-8") as f:
        json.dump(nntrainer_graph_ir, f, indent=2)
    state["nntrainer_graph_ir_path"] = graph_ir_path
    bus.log(f"Saved nntrainer graph IR to {graph_ir_path}")
