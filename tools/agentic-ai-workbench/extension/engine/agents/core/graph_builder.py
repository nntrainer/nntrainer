...
def run(state: dict) -> dict:
    ...
    # Export the raw IR (Compiler IR / nntrainer graph IR) as a standalone JSON file
    nntrainer_graph_ir = state.get("nntrainer_graph_ir")
    if nntrainer_graph_ir:
        ir_json_path = os.path.join(generated_dir, "nntrainer_graph_ir.json")
        with open(ir_json_path, "w", encoding="utf-8") as f:
            json.dump(nntrainer_graph_ir, f, indent=2)
        bus.log(f"Exported nntrainer graph IR to {ir_json_path}")

    graph_ir = state.get("graph_ir")
    if graph_ir and hasattr(graph_ir, "export_json"):
        ir_json_path = os.path.join(generated_dir, "graph_ir.json")
        with open(ir_json_path, "w", encoding="utf-8") as f:
            f.write(graph_ir.export_json())
        bus.log(f"Exported graph IR to {ir_json_path}")
    elif graph_ir and isinstance(graph_ir, dict):
        ir_json_path = os.path.join(generated_dir, "graph_ir.json")
        with open(ir_json_path, "w", encoding="utf-8") as f:
