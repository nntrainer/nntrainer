...
from ..utils.weight_enricher import enrich_nodes_with_weights
...
def _publish_semantic_graphs(state: dict) -> None:
    ...
    # Enrich nodes with weight information
    model_view["nodes"] = enrich_nodes_with_weights(model_view.get("nodes", []), state)
    nntrainer_view["nodes"] = enrich_nodes_with_weights(nntrainer_view.get("nodes", []), state)
