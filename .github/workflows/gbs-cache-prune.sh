...
# This keeps, per package (repo dir + name + arch), only the most-recently-
# downloaded .rpm and deletes older versions, bounding the cache to one snapshot.
    ...
    | awk -F'\t' '
        {
          ...
          # arch = last dot-field before .rpm; name = basename minus -ver-rel.arch.rpm
          arch = base; sub(/\.rpm$/, "", arch); sub(/.*\./, "", arch)
          name = base; sub(/-[^-]+-[^-]+\.[^.]+\.rpm$/, "", name)
          # key groups only genuine versions of one file (keep arch/dir distinct)
          key = dir name "." arch
