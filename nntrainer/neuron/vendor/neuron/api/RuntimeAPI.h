...
/**
 * Get the pitch for each dimension of the input tensor (specified by handle).
 * This function passes back the expected pitch (in pixels) in *pitch.
 * For example, when cooridnates are in the form of (N,H,W,C), pitch[3] is the number of pixels
 * between (0,0,1,c) and (0,0,0,c).
 * @param runtime The address of the created neuron runtime instance.
 * @param handle The frontend IO index.
 * @param pitch A pitch array, which size should be equal to the tensor rank.
 * @param rank The tensor rank.
 * @return A RuntimeAPI error code.
 */
int NeuronRuntime_getInputPitch(void* runtime, uint64_t handle, size_t* pitch, size_t rank);
...
/**
 * Get the pitch for each dimension of the output tensor (specified by handle).
 * This function passes back the expected pitch (in pixels) in *pitch.
 * For example, when cooridnates are in the form of (N,H,W,C), pitch[3] is the number of pixels
 * between (0,0,1,c) and (0,0,0,c).
 * @param runtime The address of the created neuron runtime instance.
 ...
 * @return A RuntimeAPI error code.
 */
int NeuronRuntime_getOutputPitch(void* runtime, uint64_t handle, size_t* pitch, size_t rank);
