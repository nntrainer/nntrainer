...
typedef enum {
    ...
    /// The tolerance time for model inferences and DVFS auto tuning.
    NEURONRUNTIME_AUTO_DVFS_TARGET_TIME,
    /// The system scenario hint, choose one from  RuntimeAPISystemScenarioHint, which is used to
    /// dynamically affect the CPU boost and DDR boost policy.
    /// Note that this setting may be overridden if the user sets NEURONRUNTIME_SYSTEM_CPU_BOOST or
    /// NEURONRUNTIME_SYSTEM_DDR_BOOST after setting NEURONRUNTIME_SYSTEM_SCENARIO_HINT.
    /// Note that if the user has set a fixed frequency or floor/ceiling settings, this QoS setting
    /// may be ineffective.
    /// @note Only valid for certain platform (e.g. DX-5 PLUS).
    NEURONRUNTIME_SYSTEM_SCENARIO_HINT,
    /// The CPU boost level for Neuron Runtime, ranged from 0 to 100. This boost value doesn't
    /// directly map to specific CPU voltage and frequency; instead, it affects the CPU boost
    /// policy.
    /// With a higher boost value, the CPU will be boosted more aggressively when a task is running.
    /// Note that if the user has set a fixed CPU frequency or floor/ceiling settings, this QoS
    /// setting may be ineffective.
    /// @note Only valid for certain platform (e.g. DX-5 PLUS).
    NEURONRUNTIME_SYSTEM_CPU_BOOST,
    /// The DDR boost level for Neuron Runtime, ranged from 0 to 100. This boost value doesn't
    /// directly map to specific DDR voltage and frequency; instead, it affects the DDR boost
    /// policy.
    /// With a higher boost value, the DDR will be boosted more aggressively.
    /// Note that if the user has set a fixed DDR frequency or floor/ceiling settings, this QoS
    /// setting may be ineffective.
    /// @note Only valid for certain platform (e.g. DX-5 PLUS).
    NEURONRUNTIME_SYSTEM_DDR_BOOST,
} RuntimeAPIQoSKind;

enum RuntimeAPISystemScenarioHint {
    NEURONRUNTIME_SYSTEM_SCENARIO_HINT_NONE = 0,
    NEURONRUNTIME_SYSTEM_SCENARIO_HINT_PERFORMANCE = 1,
    NEURONRUNTIME_SYSTEM_SCENARIO_HINT_SUSTAINABLE = 2,
    NEURONRUNTIME_SYSTEM_SCENARIO_HINT_LOWPOWER = 3,
};
...
typedef enum {
    ...
    NEURONRUNTIME_INCOMPATIBLE_DLA = 8,  ///< 8: DLA is incompatible with this system.
    NEURONRUNTIME_TIMER_EXPIRED    = 9,  ///< 9: Timer expired.
    NEURONRUNTIME_ABORTED          = 10,  ///< 10: The API is aborted.
} RuntimeAPIErrorCode;
