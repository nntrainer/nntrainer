...
bool ContextManager::CreateContext() {
  ...
  // CUDA Runtime API initializes context lazily, but here we check status
  // explicitly.
  ...
  // but if it's inactive and we just setDevice, typical runtime calls will
  // activate it. Let's force a sync to verify activation if not already.
...
#ifdef DEBUG
  std::cout << "[DEBUG] allocDeviceMemory called with size: " << size
            << std::endl;
#endif
  if (!initialized_) {
#ifdef DEBUG
    std::cout << "[DEBUG] Context not initialized, preventing creation"
              << std::endl;
#endif
    if (!CreateContext()) {
