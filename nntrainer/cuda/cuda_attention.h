 ...
 */
/**
 * @brief [cuda-kvi8] Quantize a fp16 K/V step slice into the int8 cache +
 * fp16 amax/127 scales at ring-aware rows (device kernel; matches the host
 * quantizer bit-for-bit in formula and rounding).
 */
...
/**
 * @brief [cuda-kvi8] int8-KV attention (prefill block-Q i8 / decode split-KV
 * i8) reading the int8 cache + per-(row,head_kv) fp16 scales directly.
 */
