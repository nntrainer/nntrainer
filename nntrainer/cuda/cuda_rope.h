 ...
 *        the device cuda_pos_buffer() ([0]) instead of a baked int, so a
 * captured CUDA graph stays valid across decode tokens. When @p out_slot_dpos
 * != 0, each input row is written to OUTPUT row (from + row) -- pass @p out as
 * the cache BASE pointer for the K-into-cache write so the slot is computed
