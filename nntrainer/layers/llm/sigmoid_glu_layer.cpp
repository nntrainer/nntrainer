...
void SigmoidGluLayer::incremental_forwarding(RunLayerContext &context,
  ...
  // skip-prefill gate: KV-shared attention layers skip the prefill gate.
  // Inert unless a model sets skip_prefill on sigmoid_glu. Multi-token
  // steps count as prefill (upstream 1a80cfa35), and the gate must run BEFORE
  // the step-size assert so a multi-token prefill is skipped, not thrown on.
