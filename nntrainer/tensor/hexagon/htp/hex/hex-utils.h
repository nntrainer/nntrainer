...
static inline void hex_l2fetch(const void *p, uint32_t width, uint32_t stride,
                               uint32_t height) {
...
#define HEX_L2_BLOCK_SIZE                                                      \
  (HEX_L2_LINE_SIZE * 4) // flush granularity (lines per loop iteration)
...
static inline void hex_l2flush(void *addr, size_t size) {
  ...
  const uint32_t e =
    (((uint32_t)addr) + size + HEX_L2_LINE_SIZE - 1) & ~(HEX_L2_LINE_SIZE - 1);
...
static inline void hex_pause() { asm volatile(" pause(#255)\n"); }
