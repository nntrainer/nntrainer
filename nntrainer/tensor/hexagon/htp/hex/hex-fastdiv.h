...
static inline struct fastdiv_values init_fastdiv_values(uint32_t d) {
  ...
  result.mp =
    (uint32_t)(((uint64_t)1 << 32) * (((uint64_t)1 << result.l) - d) / d + 1);
  return result;
}
...
static inline uint32_t fastdiv(uint32_t n, const struct fastdiv_values *vals) {
  ...
  const uint32_t hi =
    (uint32_t)(((uint64_t)n * vals->mp) >> 32); // mulhi(n, mp)
  // add n, apply bit shift
  return (hi + n) >> vals->l;
}

static inline uint32_t fastmodulo(uint32_t n, uint32_t d,
                                  const struct fastdiv_values *vals) {
