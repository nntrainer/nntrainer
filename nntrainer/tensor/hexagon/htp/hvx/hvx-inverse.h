...
//    least 5.9604645e-08 (32-bit code 0x33800000) and at most 131008
//    (0x47ffe000) (exponent in range [103,143])
// (2) extract the mantissa into 16-bit unsigned; find reciprocal using a fitted
// poly (3) put this, along with '253-exp' (exp from (1)) together to make an
// qf32 (4) convert that to fp16 (5) put sign back in. Also, if the original
// value (w/o sign) was <0x81, replace
...
static inline HVX_Vector hvx_vec_inverse_f16(HVX_Vector vals) {
  ...
  HVX_VectorPair to_qf32 =
    Q6_Wqf32_vmpy_VhfVhf(avals, Q6_Vh_vsplat_R(0x3C00)); // *1.0
  HVX_Vector to_f32_0 = Q6_Vsf_equals_Vqf32(Q6_V_lo_W(to_qf32));
  HVX_Vector to_f32_1 = Q6_Vsf_equals_Vqf32(Q6_V_hi_W(to_qf32));

  // bits 22..13 contain the mantissa now (w/o hidden bit); move to bit 14..5 of
  // a 16-bit vector
  HVX_Vector mant_u16 = Q6_Vh_vshuffo_VhVh(Q6_Vw_vasl_VwR(to_f32_1, 9),
                                           Q6_Vw_vasl_VwR(to_f32_0, 9));
  // likewise extract the upper 16 from each, containing the exponents in range
  // 103..142
  ...
  // so, mant_u16 contains an unbiased mantissa in upper 10 bits of each u16
  // lane We can consider it to be x-1.0, with 16 fractional bits, where 'x' is
  // in range [1.0,2.0) Use poly to transform to 1/x, with 14 fractional bits
  //
  ...
  HVX_Vector mant_recip = Q6_V_vand_VV(
    Q6_Vh_vasr_VhR(Q6_Vh_vasl_VhVh(rm, vcl0), 5), Q6_Vh_vsplat_R(0x03FF));

  // Compute Reciprocal Exponent
  HVX_Vector exp_recip =
    Q6_Vh_vsub_VhVh(Q6_Vh_vsub_VhVh(Q6_Vh_vsplat_R(254), exp_u16),
                    Q6_Vh_vsub_VhVh(vcl0, Q6_Vh_vsplat_R(1)));
  // Convert it for 16-bit representation
  exp_recip = Q6_Vh_vadd_VhVh_sat(
    Q6_Vh_vsub_VhVh(exp_recip, Q6_Vh_vsplat_R(127)), Q6_Vh_vsplat_R(15));
...
static inline HVX_Vector hvx_vec_inverse_f32(HVX_Vector v_sf) {
  ...
  r_qf = Q6_Vqf32_vmpy_VsfVsf(
    i_sf, Q6_Vsf_equals_Vqf32(Q6_Vqf32_vsub_VsfVsf(
            two_sf, Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(i_sf, v_sf)))));
  r_qf = Q6_Vqf32_vmpy_Vqf32Vqf32(
    r_qf, Q6_Vqf32_vsub_VsfVsf(two_sf, Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(
                                         Q6_Vsf_equals_Vqf32(r_qf), v_sf))));
  r_qf = Q6_Vqf32_vmpy_Vqf32Vqf32(
    r_qf, Q6_Vqf32_vsub_VsfVsf(two_sf, Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(
                                         Q6_Vsf_equals_Vqf32(r_qf), v_sf))));
...
static inline HVX_Vector hvx_vec_inverse_f16_guard(HVX_Vector v_sf,
                                                   HVX_Vector nan_inf_mask) {
...
#define DEFINE_HVX_INV_OP_VARIANTS(OP_NAME, OP_LOOP_BODY)                      \
  static inline void OP_NAME##_aa(uint8_t *restrict dst,                       \
                                  const uint8_t *restrict src, uint32_t n) {   \
    assert((uintptr_t)dst % 128 == 0);                                         \
    assert((uintptr_t)src % 128 == 0);                                         \
    OP_LOOP_BODY(HVX_Vector, HVX_Vector, hvx_vec_store_a);                     \
  }                                                                            \
  static inline void OP_NAME##_au(uint8_t *restrict dst,                       \
                                  const uint8_t *restrict src, uint32_t n) {   \
    assert((uintptr_t)dst % 128 == 0);                                         \
    OP_LOOP_BODY(HVX_Vector, HVX_UVector, hvx_vec_store_a);                    \
  }                                                                            \
  static inline void OP_NAME##_ua(uint8_t *restrict dst,                       \
                                  const uint8_t *restrict src, uint32_t n) {   \
    assert((uintptr_t)src % 128 == 0);                                         \
    OP_LOOP_BODY(HVX_UVector, HVX_Vector, hvx_vec_store_u);                    \
  }                                                                            \
  static inline void OP_NAME##_uu(uint8_t *restrict dst,                       \
                                  const uint8_t *restrict src, uint32_t n) {   \
...
#define HVX_INV_DISPATCHER(OP_NAME)                                            \
  static inline void OP_NAME(uint8_t *restrict dst,                            \
                             const uint8_t *restrict src,                      \
                             const uint32_t num_elems) {                       \
    if (hex_is_aligned((void *)dst, 128) &&                                    \
        hex_is_aligned((void *)src, 128)) {                                    \
