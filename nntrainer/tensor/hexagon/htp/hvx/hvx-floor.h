...
static inline HVX_Vector hvx_vec_truncate_f32(HVX_Vector in_vec) {
  ...
  HVX_Vector rshift_v =
    IEEE_VSF_MANTLEN - expval_v; // fractional bits - exp shift
...
static inline HVX_Vector hvx_vec_floor_f32(HVX_Vector in_vec) {
  ...
  HVX_VectorPred q_negexp_pos =
    Q6_Q_vcmp_gtand_QVwVw(q_negexp, in_vec, const_zero_v);
  HVX_VectorPred q_negexp_neg =
    Q6_Q_vcmp_gtand_QVwVw(q_negexp, const_zero_v, in_vec);
  ...
  //    if (qneg)                     // negative floor is one less (more, sign
  //    bit for neg)
  ...
  vout =
    Q6_V_vmux_QVV(q_negexp_neg, const_negone_v, vout); // expval<0 x<0 -> -1
