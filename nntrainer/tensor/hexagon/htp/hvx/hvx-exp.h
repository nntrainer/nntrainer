...
static inline HVX_Vector hvx_vec_exp_f32(HVX_Vector in_vec) {
  ...
  //          = exp(epsilon)*2^f
  //
  //   Since epsilon is close to zero, it can be approximated with its Taylor
  //   series:
  ...
  HVX_VectorPred pred_cap_right =
    Q6_Q_vcmp_gt_VsfVsf(in_vec, Q6_V_vsplat_R(EXP_RANGE_R));
  HVX_VectorPred pred_cap_left =
    Q6_Q_vcmp_gt_VsfVsf(Q6_V_vsplat_R(EXP_RANGE_L), in_vec);
...
hvx_vec_exp_f32_guard(HVX_Vector in_vec, HVX_Vector max_exp, HVX_Vector inf) {
...
static inline void hvx_exp_f32(uint8_t *restrict dst,
                               const uint8_t *restrict src, const int num_elems,
                               bool negate) {
...
#pragma unroll(4)
    for (int i = 0; i < num_elems_whole; i += VLEN_FP32) {
      ...
      if (true == negate) {
        ...
        *(HVX_UVector *)(dst + i * SIZEOF_FP32) =
          hvx_vec_exp_f32_guard(neg_vec_in, max_exp, inf);
      } else {
        *(HVX_UVector *)(dst + i * SIZEOF_FP32) =
          hvx_vec_exp_f32_guard(in, max_exp, inf);
      }
    }
  }
...
static inline HVX_Vector hvx_vec_exp2_f16(HVX_Vector x_v) {
  ...
  HVX_Vector x_minus_half =
    Q6_Vhf_equals_Vqf16(Q6_Vqf16_vsub_VhfVhf(x_v, half_hf_v));
  ...
  HVX_Vector y =
    Q6_Vqf16_vmpy_Vqf16Vqf16(Q6_Vh_vsplat_R(0x5082), x_qf16); // E5*x
  ...
  // Combine polynomial (mantissa) with integer part (exponent): result = y *
  // 2^k
