...
#ifdef ENABLE_CUDA
  {
    // Clear any previous errors to ensure we catch the error from this call
    cudaGetLastError();

    cudaPointerAttributes attr_src, attr_dest;
    cudaError_t err_src = cudaPointerGetAttributes(&attr_src, buf);
    cudaError_t err_dest = cudaPointerGetAttributes(&attr_dest, getData());

    cudaMemcpyKind kind = cudaMemcpyDefault;
    std::string src_type = "Unknown", dest_type = "Unknown";

    if (err_src == cudaSuccess) {
      src_type = (attr_src.type == cudaMemoryTypeDevice) ? "Device" : "Host";
    }
    if (err_dest == cudaSuccess) {
      dest_type = (attr_dest.type == cudaMemoryTypeDevice) ? "Device" : "Host";
    }

    // Explicitly determine kind
    if (src_type == "Device" && dest_type == "Device")
      kind = cudaMemcpyDeviceToDevice;
    else if (src_type == "Host" && dest_type == "Device")
      kind = cudaMemcpyHostToDevice;
    else if (src_type == "Device" && dest_type == "Host")
      kind = cudaMemcpyDeviceToHost;
    else if (src_type == "Host" && dest_type == "Host")
      kind = cudaMemcpyHostToHost;

    cudaError_t err = cudaMemcpy(getData(), buf, size() * sizeof(float), kind);

    if (err != cudaSuccess) {
      std::cerr << "[ERROR] FloatTensor::copy cudaMemcpy failed: "
                << cudaGetErrorString(err) << " (code " << (int)err << ") "
                << "src=" << buf << " dest=" << getData() << " size=" << size()
                << std::endl;

      if (kind != cudaMemcpyDefault) {
        err =
          cudaMemcpy(getData(), buf, size() * sizeof(float), cudaMemcpyDefault);
        if (err != cudaSuccess) {
          std::cerr << "[ERROR] Retry also failed: " << cudaGetErrorString(err)
                    << std::endl;
        }
      }
    }
