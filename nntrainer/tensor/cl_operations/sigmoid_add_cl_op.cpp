 ...
 *
 * Relocated verbatim from sigmoid_add_cl.cpp
 * (SigmoidAddLayerCl::sigmoid_addProcess / sigmoid_add_cl / sigmoid_add_cl_fp16
 * / registerClKernels) into free functions so the SigmoidAdd layer can be a
 * single backend-neutral Layer.
 */
...
// Exposed (declared in sigmoid_add_cl_op.h) for the OpenCL kernel
// micro-benchmarks; the layer dispatch goes through sigmoid_add_cl_op() below.
void sigmoid_add_cl(float *matAdata, float *vecXdata, float *vecYdata,
  ...
  do {
    const auto &kernel_sigmoid_add_ptr =
      getLayerKernelPtrs()[Kernels::SIGMOID_ADD_CL];
    ...
    if (!svm) {
      ...
      set_result &= kernel_sigmoid_add_ptr->SetKernelArguments(0, &bufferInA,
                                                               sizeof(cl_mem));
      set_result &= kernel_sigmoid_add_ptr->SetKernelArguments(1, &bufferInB,
                                                               sizeof(cl_mem));
      set_result &= kernel_sigmoid_add_ptr->SetKernelArguments(2, &bufferOutA,
                                                               sizeof(cl_mem));
      if (!set_result) {
        break;
      }
      ...
      set_svm_result &=
        kernel_sigmoid_add_ptr->SetKernelSVMArguments(0, matAdata);
      set_svm_result &=
        kernel_sigmoid_add_ptr->SetKernelSVMArguments(1, vecXdata);
      set_svm_result &=
        kernel_sigmoid_add_ptr->SetKernelSVMArguments(2, vecYdata);
      if (!set_svm_result) {
  ...
  do {
    ...
    if (!svm) {
      ...
      if (in1_clmem != nullptr) {
        ...
        set_svm_result &= kernel_sigmoid_add_ptr->SetKernelArguments(
          0, &in1_buf, sizeof(cl_mem));
      } else {
        set_svm_result &=
          kernel_sigmoid_add_ptr->SetKernelSVMArguments(0, matAdata);
      }
    ...
    if (!svm) {
      ...
      // async: sigmoid_add output is consumed by the next GPU op (ffn_down FC);
      // the in-order queue preserves ordering, no host read between.
      if (!global_cl_context->command_queue_inst_.enqueueSVMMap(
...
bool registerSigmoidAddClKernels(ClContext &cl_context) {
  ...
  do {
    ...
    kernel_sigmoid_add_ptr =
      cl_context.registerClKernel(sigmoid_add_kernel, "sigmoid_add_cl");
...
#ifdef ENABLE_FP16
    kernel_sigmoid_add_ptr = cl_context.registerClKernel(
      sigmoid_add_fp16_kernel, "sigmoid_add_cl_fp16");
...
void sigmoid_add_cl_op(const Tensor &in1, const Tensor &in2, Tensor &result,
  ...
  if (in1.getDataType() == ml::train::TensorDim::DataType::FP32) {
    ...
    // cl_mem handle as a host pointer => the sigmoid_add output is never
    // written => the FFN contributes nothing to the residual (measured: qwen3
    // MLP drops out, layer-0 output collapses to embed+attention).
