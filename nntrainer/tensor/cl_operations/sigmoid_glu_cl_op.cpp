 ...
 *
 * Relocated verbatim from sigmoid_glu_cl.cpp
 * (SigmoidGluLayerCl::sigmoid_gluProcess / sigmoid_glu_cl / sigmoid_glu_cl_fp16
 * / registerClKernels) into free functions so the SigmoidGlu layer can be a
...
// Exposed (declared in sigmoid_glu_cl_op.h) for the OpenCL kernel
// micro-benchmarks; the layer dispatch goes through sigmoid_glu_cl_op() below.
void sigmoid_glu_cl(float *matAdata, float *vecXdata, float *vecYdata,
  ...
  do {
    const auto &kernel_sigmoid_glu_ptr =
      getLayerKernelPtrs()[Kernels::SIGMOID_GLU_CL];
    ...
    if (!svm) {
      ...
      set_result &= kernel_sigmoid_glu_ptr->SetKernelArguments(0, &bufferInA,
                                                               sizeof(cl_mem));
      set_result &= kernel_sigmoid_glu_ptr->SetKernelArguments(1, &bufferInB,
                                                               sizeof(cl_mem));
      set_result &= kernel_sigmoid_glu_ptr->SetKernelArguments(2, &bufferOutA,
                                                               sizeof(cl_mem));
      if (!set_result) {
        break;
      }
      ...
      set_svm_result &=
        kernel_sigmoid_glu_ptr->SetKernelSVMArguments(0, matAdata);
      set_svm_result &=
        kernel_sigmoid_glu_ptr->SetKernelSVMArguments(1, vecXdata);
      set_svm_result &=
        kernel_sigmoid_glu_ptr->SetKernelSVMArguments(2, vecYdata);
      if (!set_svm_result) {
  ...
  do {
    ...
    if (!svm) {
      ...
      if (in1_clmem != nullptr) {
        ...
        set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelArguments(
    ...
    if (!svm) {
      ...
      // async: sigmoid_glu output is consumed by the next GPU op (ffn_down FC);
...
bool registerSigmoidGluClKernels(ClContext &cl_context) {
  ...
  do {
    ...
    kernel_sigmoid_glu_ptr =
      cl_context.registerClKernel(sigmoid_glu_kernel, "sigmoid_glu_cl");
...
#ifdef ENABLE_FP16
    kernel_sigmoid_glu_ptr = cl_context.registerClKernel(
      sigmoid_glu_fp16_kernel, "sigmoid_glu_cl_fp16");
...
void sigmoid_glu_cl_op(const Tensor &in1, const Tensor &in2, Tensor &result,
  ...
  if (in1.getDataType() == ml::train::TensorDim::DataType::FP32) {
    ...
    // cl_mem handle as a host pointer => the sigmoid_glu output is never
