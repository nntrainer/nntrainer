...
void sigmoid_glu_cl(const float *in1, const float *in2, float *out,
  ...
  dispatchGatedClKernel<float>(getOpKernelPtrs()[Kernels::SIGMOID_GLU_CL], in1,
...
#ifdef ENABLE_FP16
    kernel_ptr = cl_context.registerClKernel(sigmoid_glu_fp16_kernel,
                                             "sigmoid_glu_cl_fp16");
    if (!kernel_ptr) {
...
void sigmoid_glu_cl_op(const Tensor &in1, const Tensor &in2, Tensor &out,
  ...
  if (getOpKernelPtrs().empty())
    throw std::runtime_error("sigmoid_glu_cl_op: the OpenCL kernels are not "
  ...
  if (dt == ml::train::TensorDim::DataType::FP32) {
    ...
    dispatchGatedClKernel<_FP16>(
      getOpKernelPtrs()[Kernels::SIGMOID_GLU_CL_FP16],
    ...
    throw std::invalid_argument("sigmoid_glu_cl_op: unsupported data type");
  }
}
