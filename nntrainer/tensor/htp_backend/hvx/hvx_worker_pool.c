...
static void hvx_worker_pool_thread_entry(void *arg) {
  ...
  for (;;) {
    ...
    // destroy() bumps seqn purely to wake sleepers, so a worker that got
    // preempted between the check at the top of this loop and the load
    // above would otherwise re-run the PREVIOUS job here -- with a ctx
    // whose caller frame is already gone. destroy() writes killed before
    // it releases seqn, so seeing the bump means seeing the kill.
    if (atomic_load_explicit(&pool->killed, memory_order_relaxed)) {
      qurt_thread_exit(0);
    }
