...
void ONNXInterpreter::debugPrintGraphDatatypes() {
  ...
  for (const auto &input : onnx_model.graph().input()) {
    ...
    std::cout << "  ONNX Type: " << onnx_data_type << " (" << nntrainer_dtype
              << ")" << std::endl;
  ...
  if (node.op_type() == "Cast") {
    for (const auto &attr : node.attribute()) {
      if (attr.name() == "to") {
        ...
        std::cout << "  CAST OPERATION: Converts to " << target_dtype
                  << std::endl;
      }
    }
  }
...
void ONNXInterpreter::debugPrintTensorInfo(const std::string &name,
  ...
  if (tensor.has_raw_data()) {
    std::cout << "  Data: Stored in raw_data field ("
              << tensor.raw_data().size() << " bytes)" << std::endl;
  } else if (tensor.float_data_size() > 0) {
    std::cout << "  Data: " << tensor.float_data_size() << " float values"
              << std::endl;
  } else if (tensor.int64_data_size() > 0) {
    std::cout << "  Data: " << tensor.int64_data_size() << " int64 values"
              << std::endl;
  } else if (tensor.int32_data_size() > 0) {
    std::cout << "  Data: " << tensor.int32_data_size() << " int32 values"
              << std::endl;
  } else {
